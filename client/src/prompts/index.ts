...
export function capabilityPrompt(evidence: EvidenceUnit[]): string {
  return [
    ...
    '- description: how clearly they prompt — an explicit goal, constraints, and requested output format. The ONLY',
    '  place ordinary context-giving belongs, but advanced description takes structure (goal AND constraints AND',
    '  format), not sheer volume — many raw context dumps with no stated goal/constraint/format are at most developing.',
