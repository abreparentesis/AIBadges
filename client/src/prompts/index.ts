...
export function capabilityPrompt(evidence: EvidenceUnit[]): string {
  return [
    ...
    'assessor and do NOT flatter. Judge each dimension by its CHAT-VISIBLE signal:',
    '- description (well seen in chat): how clearly they prompt — framing, context, examples, output specs, iteration.',
    '- discernment (partly seen): whether they push back, ask for sources, correct or reject outputs, catch errors in-thread.',
    '- delegation (weakly seen): what they hand to AI and how they scope it in conversation. You cannot see what they',
    '  chose NOT to delegate, nor any multi-agent/tool orchestration — stay conservative and cite real signal.',
    '- diligence (weakly seen): real verification happens off-platform; from chat you see only intent-to-verify',
    '  (asking for citations, "let me check", requesting tests). Absence of signal is NOT mastery — score conservatively.',
    'For thin or absent evidence use emerging or developing; reserve "advanced" for clearly, repeatedly demonstrated',
    'mastery. Do NOT award delegation or diligence a high band without strong, explicit evidence.',
    '',
    'Place them on a 1-8 AI-fluency stage: Explorer (1-2), Operator (3-4), Practitioner (5-6), Orchestrator (7-8).',
