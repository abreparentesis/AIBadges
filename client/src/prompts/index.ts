...
export function capabilityPrompt(evidence: EvidenceUnit[]): string {
  return [
    ...
    'For EACH dimension also write a one-sentence "note": the recurring pattern that earns the band, in plain terms.',
    ...
    '{"aiFluency":{"delegation":{"band":..,"note":"..","evidenceIds":[..]},"description":{..},"discernment":{..},"diligence":{..}},',
...
export function capabilityAuditPrompt(evidence: EvidenceUnit[], draft: Capability): string {
  ...
  return [
    ...
    'merely rejecting a suggestion). When in doubt choose the LOWER band. For each dimension write a one-sentence',
    '"note" stating the pattern that earns the band; the band MUST match the note.',
