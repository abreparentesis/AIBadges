...
export function capabilityPrompt(evidence: EvidenceUnit[]): string {
  return [
    ...
    '- description: how clearly they prompt — goal, context, constraints, output format. The ONLY place context-giving belongs.',
    '- discernment: the person REACTING TO the model\'s output — correcting it, disagreeing, rejecting or redoing a weak',
    '  answer, catching an error, narrowing an over-broad reply. Stating one\'s own facts or opinions is NOT discernment.',
    ...
    '  evidenceIds (or []) — do NOT pad it with unrelated asks or with context the person supplied.',
    ...
    'not show that dimension\'s specific engagement with the AI — most often fact/context statements wrongly placed under',
    'discernment, diligence, or delegation. Then set each band to what the SURVIVING quotes support (0-1 genuine → emerging/developing).',
    '',
