...
export function capabilityPrompt(evidence: EvidenceUnit[]): string {
  return [
    ...
    '  — do not under-rate it. But a BARE IMPERATIVE with nothing attached ("do a critical analysis of this",',
    '  "evalúa esto") is a handoff in form only: on its own it is developing at most. Merely naming a task\'s',
    '  subject is not delegation. A chat rarely shows the deliberate',
    ...
    'Score each band from the OVERALL PATTERN across the whole history, NOT the number of quotes. Ladder: emerging =',
    'essentially no sign of the skill; developing = only occasional or shallow instances (rejecting a suggestion,',
    'refining a request, a one-line fix), even if there are several; proficient = a regular, substantive pattern across',
    'different conversations; advanced = a defining, SOPHISTICATED pattern that recurs (for discernment, e.g., catching',
    'subtle errors, challenging the model\'s reasoning, or re-framing its approach — never a handful of shallow',
    'reactions). A few lousy, short, or ambiguous quotes NEVER earn proficient or advanced; when torn, go LOWER.',
    'For EACH dimension also write a one-sentence "note": the recurring pattern that earns the band, written FOR THE',
    'USER in plain second person ("You regularly push back when...", "You rarely say what done looks like..."). NEVER',
    'mention evidence ids (e1, e34), quote counts, or any internal bookkeeping — the note is a diagnosis a',
    'non-technical person can understand, not an audit trail.',
    'The band MUST match the note — if you cannot describe a genuinely sophisticated, recurring pattern, it is not',
    'advanced; if you cannot describe a real pattern at all, it is emerging or developing.',
    'For EACH dimension also write a one-sentence "nextStep": the single most useful concrete action THIS user could',
    'take next, grounded in their own behavior (e.g. "When you hand a task off, state the budget and deadline you',
    'usually leave implicit"). Personal and immediately actionable — never generic advice like "practice prompting".',
    'SELF-AUDIT before output, two passes. (1) Per quote: delete any cited quote that, read alone, does not show that',
    'dimension\'s specific engagement with the AI — for discernment/diligence it MUST point to something the AI said or',
    'produced; for delegation it MUST contain the actual handed-off task, not its topic. (2) Holistic: look at the',
    'survivors as a whole — if the pattern is shallow, sparse, or ambiguous, LOWER the band and rewrite the note.',
    ...
    '{"aiFluency":{"delegation":{"band":..,"note":"..","nextStep":"..","evidenceIds":[..]},"description":{..},"discernment":{..},"diligence":{..}},',
...
export function capabilityAuditPrompt(evidence: EvidenceUnit[], draft: Capability): string {
  ...
  return [
    ...
    '  for a fact is not delegating a task). A BARE IMPERATIVE with nothing attached ("do a critical analysis',
    '  of this", "evalúa esto") is a handoff in form only — it may stay as evidence, but on its own it is',
    '  developing at most; proficient requires briefs that carry their own scope, constraints, or decision.',
    ...
    '  claim it made, test its output, report back that its suggestion failed). DELETE fresh factual questions,',
    '  one-word asks, and FIRST-MESSAGE requests to cite sources ("asegúrate de citar fuentes") — demanding',
    '  sources up front is description at most; diligence begins when the person CHECKS what came back.',
    'REUSE: when the SAME quote is cited by two dimensions, keep it only in the dimension it evidences BEST',
    'and delete it from the other, then re-band both from what remains. A small set of quotes doing double',
    'duty across dimensions is how thin evidence masquerades as breadth.',
    'RE-BAND from the OVERALL PATTERN of the survivors, not the count: step back and judge them as a whole. A few',
    'shallow, short, or ambiguous quotes are developing at most — advanced and proficient require a genuinely',
    'SOPHISTICATED, recurring pattern (discernment: catching subtle errors or challenging the model\'s reasoning, not',
    'merely rejecting a suggestion). When in doubt choose the LOWER band. For each dimension REWRITE the one-sentence',
    '"note" to match the surviving evidence, in plain second person for the user — NEVER mention evidence ids, quote',
    'counts, or bookkeeping. Also rewrite (or keep, if still accurate) the "nextStep": one concrete, personal action',
    'grounded in their own behavior, never generic advice. The band MUST match the note.',
    'FINALLY, sanity-check the four bands against each other. It is rare to be equally strong at all four, so if three',
    'or four came out advanced you are almost certainly over-rating — keep advanced only where the evidence is genuinely',
    'sophisticated and recurring. But do NOT mechanically force a spread: a dimension that honestly earns proficient',
    'STAYS proficient even when another is advanced. Each band must match its own note and surviving quotes, neither',
    'inflated to match the others nor deflated below what its evidence shows. Audit domains too.',
