...
const SYNTHESIS_STEPS = [
  ...
  '- delegation = handing off whole, well-scoped tasks and choosing what to delegate. Handing the model substantial self-contained jobs (e.g. "evaluate this opportunity end to end", "produce a market study and business plan") is proficient or advanced — do not under-rate it. Merely naming a task\'s subject is not delegation.',
  '- description = how clearly they prompt: goal, context, constraints, output format. This is the ONLY dimension where ordinary context-giving belongs.',
  ...
  '- diligence = verifying before acting: asking for sources, cross-checking, testing, iterating. Hardest to see in chat; if the quotes do not actually show verification, score it emerging or developing and cite only truly relevant quotes (or []). Do NOT pad it with unrelated asks or with context the person supplied.',
  ...
  'SELF-AUDIT before you output aiFluency: re-read every quote you cited under each dimension and DELETE any that, read alone, does not show that dimension\'s specific engagement with the AI — most often fact or context statements wrongly placed under discernment, diligence, or delegation. Then set each band to what the SURVIVING quotes support; if a dimension is left with 0-1 genuine quotes it is emerging or developing.',
  'Place the person on a 1-8 fluency stage matching the strongest consistently-evidenced dimensions; chat caps the stage at 6 (reserve 7-8 only for explicit agent or tool orchestration). List only domains the evidence supports.',
].join('\n');
