...
const SYNTHESIS_STEPS = [
  ...
  'For EACH dimension also write a one-sentence "note": the actual recurring pattern that earns the band, written FOR THE USER in plain second person (e.g. "You regularly challenge the model\'s reasoning and ask it to redo weak answers"). NEVER mention evidence ids (e1, e34), quote counts, or any internal bookkeeping — the note is a diagnosis a non-technical person can understand and act on, not an audit trail. The band MUST match the note — if you cannot describe a genuinely sophisticated, recurring pattern, it is not advanced; if you cannot describe a real pattern at all, it is emerging or developing.',
  'For EACH dimension also write a one-sentence "nextStep": the single most useful concrete action THIS user could take next, grounded in their own behavior (e.g. "When you hand a task off, state the budget and deadline you usually leave implicit"). Personal and immediately actionable — never generic advice like "practice prompting".',
...
const AUDIT_RULES = [
  ...
  '- delegation: keep only a handed-off, self-contained TASK ("evaluate this opportunity end to end", "produce a comparative table on X"). DELETE plain information questions ("how is the forensic-medicine postgrad at X?", "what is the real parking demand at X?") — asking for a fact is not delegating a task. A BARE IMPERATIVE with nothing attached ("do a critical analysis of this", "evalúa esto") is a handoff in form only — it may stay as evidence, but on its own it is developing at most; proficient requires briefs that carry their own scope, constraints, or decision.',
  ...
  '- diligence: keep only quotes that verify what the AI GAVE (challenge a source it cited, cross-check a claim it made, test its output, report back that its suggestion failed). DELETE fresh factual questions, one-word asks ("Validate", "does income tax rise with income?"), and FIRST-MESSAGE requests to cite sources ("asegúrate de citar fuentes") — demanding sources up front is description at most; diligence begins when the person CHECKS what came back.',
  'REUSE: when the SAME quote is cited by two dimensions, keep it only in the dimension it evidences BEST and delete it from the other, then re-band both from what remains. A small set of quotes doing double duty across dimensions is how thin evidence masquerades as breadth.',
  'Then RE-BAND each dimension from the OVERALL PATTERN of what survives, not the count: step back and judge the surviving quotes as a whole. A few shallow, short, or ambiguous quotes are developing at most — advanced and proficient require a genuinely SOPHISTICATED, recurring pattern (e.g. discernment: catching subtle errors or challenging the model\'s reasoning, not merely rejecting a suggestion or refining a request). When in doubt choose the LOWER band. For each dimension REWRITE the one-sentence "note" to match the surviving evidence, in plain second person for the user — NEVER mention evidence ids, quote counts, or bookkeeping. Also rewrite (or keep, if still accurate) the "nextStep": one concrete personal action grounded in their behavior, never generic advice. The band MUST match the note.',
  'FINALLY, sanity-check the four bands against each other. It is rare to be equally strong at all four, so if three or four came out "advanced" you are almost certainly over-rating — keep advanced only where the evidence is genuinely sophisticated and recurring (shallow reactions like "redo it" or rejecting a suggestion are never advanced discernment). But do NOT mechanically force a spread: a dimension that honestly earns proficient STAYS proficient even when another is advanced. Each band must match its own note and surviving quotes — neither inflated to match the others nor deflated below what its evidence shows. Audit domains the same way.',
].join('\n');
const AUDIT_SHAPE = '{"aiFluency":{"delegation":{"band":"emerging|developing|proficient|advanced","note":"pattern, second person, no ids","nextStep":"one concrete personal action","evidenceIds":["e1"]},"description":{...},"discernment":{...},"diligence":{...}},"domains":[{"name":"...","band":"...","evidenceIds":["e1"]}]}';
